...
clock = pygame.time.Clock() #יצירת אובייקט שעוזר לנו לשלוט בקצב הריצה של המשחק, כדי שהמשחק לא יתקדם מהר מדי ויאפשר לשחקנים לראות את המהלכים בצורה נוחה

graphics = Graphics() #יצירת אובייקט גרפיקה שאחראי על הצגת המשחק, כולל ציור הלוח, האבנים והטקסטים השונים במהלך המשחק
env = Environment(State()) #יצירת אובייקט סביבה שמכיל את המצב הנוכחי של המשחק ומאפשר לבצע מהלכים ולבדוק את חוקיותם, כמו גם לבדוק אם המשחק נגמר ומי המנצח

#יצירת שני שחקנים אנושיים, אחד לכל צד של הלוח. כל שחקן מקבל את הסביבה והגרפיקה כדי שיוכל לבדוק את חוקיות המהלכים ולחשב את הפעולה המתאימה ללחיצה על המסך
